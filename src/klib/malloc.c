...
ki_malloc(int size)
{
...
#if NUMSMALL
  // gestisci i blocchi di dimensione inferiore ad ALIGN 
  // attraverso una lista divisa per dimensione
  if (size < SMALL)
  {
  ...
  // ho una freelist per ogni bucket
  b = size2bucket(size);
  prev = &(freelist[b]);
...
ki_free(void *ptr)
{
  ...
  {
  char buf[100];
  sprintf(buf,"free(%08x:%d)", block,block->size);
  log_message(buf);
  }
#if DEBUG
  printf("free(%u/%08x)\n", block->size, block);
...
void *
ki_realloc(void *ptr, size_t size)
{
  BLOCK *b;
  char *newptr;
  size_t copysize;
...
void ki__malloc(int size, void * * p)
{
	*p = ki_malloc(size);
}
