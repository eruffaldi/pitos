 ...
 *            is simulated environment that uses the file system to
...
static void getStoredFileName(char * buf, char *name);
static void initTotalSpace(void);
...
int JamGetAppJarSize(JamApp *app) {
    ...
    getStoredFileName(path, app->jarName);
...
char *JamGetCurrentJar() {
    static char currentJar[MAX_PATH];

    getStoredFileName(currentJar, JamGetCurrentApp()->jarName);
    return currentJar;
}
...
int JamOpenAppsDatabase() {
    char path[MAX_PATH];
    getStoredFileName(path, "list.txt");
