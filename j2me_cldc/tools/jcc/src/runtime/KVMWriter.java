/*
 *		KVMWriter.java		1.24		00/05/05		SMI
 *
...
public class KVMWriter implements CoreImageWriter, Const, EVMConst {
    ...
    protected void writeAllClassDeclarations(ClassClass classes[]) 
        ...
        for (Enumeration e = classTable.enumerate(); e.hasMoreElements(); ) { 
            ...
            if (ci == null) { 
                if (name.charAt(0) == '[') { 
                    ...
                    out.println(FORWARD_STATIC_DECLARATION + 
                            "static struct instanceClassStruct "
                            + myName + "_Classblock;");
                    continue;
                }
            }
            ...
            if (cc.isPrimitiveClass()) { 
                ...
                // Write an array class declaration.
                // We have to make sure that the class is explicitly added to
                // the class table.
                out.println(FORWARD_STATIC_DECLARATION +
    ...
    protected void writeNormalClassDefinition(EVMClass c) {
        ...
        if (c.ci.superClass == null) {
            ...
            out.println("\t\t&" + superName + "_Classblock, \\");
        }
        out.println("\t\t" 
                    + ((methodCount==0) ? "NULL" 
	...
	out.println("#if !ENABLEFASTBYTECODES");
	out.println("CONST");
	out.println("#endif");
	...
	out.println("\t\t\t{");
        for (int i = 0; i < methodCount; i++) { 
            ...
            MethodInfo mi = meth.method;
	    String type = ((mi.access & Const.ACC_ABSTRACT) != 0) ? "ABSTRACT_"
		        : ((mi.access & Const.ACC_NATIVE) != 0)   ? "NATIVE_"
		        : "";
		...
		out.println("\t\t\t\t\tAllCode." 
            ...
            if ((mi.access & (Const.ACC_ABSTRACT + Const.ACC_NATIVE)) != 0) { 
                ...
                out.println(mi.locals + ", " + mi.stack + ", " + mi.code.length + ", \\");
		out.print("\t\t\t\t\t");
            }
