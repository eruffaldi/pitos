...
/*
 * Access local hash tables without locking.  This extra level of
 * naming is intended to emphasize the fact that locks are not held,
 * as well as to do error handling.  Although it's not strictly necessary,
 * you should always use the "_Local" versions of these functions on the
 * verifier's local hash tables.
 *
 * These things would all be macros, like Str2IDFree_Local(), except
 * that Str2ID() and ID2Str() can run out of memory, and the code in
 * this file makes it inconvenient to check for that.  As an expedient,
 * rather than throwing exceptions, Str2ID_Local() and ID2Str_Local()
 * are functions that call CCerror() if necessary.
 */
...
bool_t verify_class_codes(ClassClass *cb) {
	...
	*addr = cb;
	...
	for (i = cbFieldsCount(cb), fb = cbFields(cb); --i >= 0; fb++) 
        ...
        unhand(context->class)->extra_utf8_entries = 
	    (char **)malloc((cbConstantPoolCount(context->class) * 3 + 100) * sizeof(char *));
        unhand(context->class)->n_extra_utf8_entries = 1;
        unhand(context->class)->extra_utf8_entries[0] = "StackMap";

	for (i = cbMethodsCount(cb), mb = cbMethods(cb); --i >= 0; mb++) 
	    verify_method(context, mb);

	unhand(context->class)->extra_utf8_entries = 
	    (char **)realloc(unhand(context->class)->extra_utf8_entries,
	                     unhand(context->class)->n_extra_utf8_entries * sizeof(char *));
...
struct map_entry get_type_code(context_type *context, fullinfo_type type)
{
	...
	if (GET_ITEM_TYPE(type) == ITEM_NewObject) {
	    ...
	    for (i = 1; i < unhand(context->class)->n_extra_utf8_entries; i++) {
		if (strcmp(type_name, unhand(context->class)->extra_utf8_entries[i]) == 0) {
		    result.info = - i;
		    return result;
		}
	    }
	    unhand(context->class)->extra_utf8_entries[unhand(context->class)->n_extra_utf8_entries] = strdup(type_name);
	    result.info = 0 - unhand(context->class)->n_extra_utf8_entries;
            unhand(context->class)->n_extra_utf8_entries++;
	    return result;
	} else {
	    panic("bad type code");
...
verify_method(context_type *context, struct methodblock *mb)
{
	...
	this_idata->length = length;
	{
		...
		if (inline_jsr_on) {
			this_idata->is_target = mb->mp_marks[offset];
		} else {
			this_idata->is_target = 0;
		}
	}
    ...
    mb->stack_maps = (struct stack_map *)malloc(n_stack_maps * sizeof(struct stack_map));

    n_stack_maps = 0;

    for (inumber = 0; inumber < instruction_count; inumber++) {
	instruction_data_type *this_idata = &idata[inumber];
	if (this_idata->not_dead == 0) {
	    int index;
	    for (index = this_idata->offset;
		 index < this_idata->offset + this_idata->length;
		 index++) {
		mb->code[index] = opc_nop;
	    }
	    /* redo inline jsr to eliminate dead nops (these don't have
	     * the right stack maps. */
	    unhand(mb->fb.clazz)->redo = 1;
	}
	if (this_idata->is_target) {
	    ...
	    mb->stack_maps[n_stack_maps].nlocals = 0;
	    for (index = 0; index < register_info->register_count; index++) {
		if (register_info->registers[index] != ITEM_Double_2 &&
		    register_info->registers[index] != ITEM_Long_2) {
		    mb->stack_maps[n_stack_maps].nlocals++;
		}
	    }
	    new_entries = (struct map_entry *)malloc(mb->stack_maps[n_stack_maps].nlocals * sizeof(struct map_entry));
	    for (index2 = 0, index = 0; index < register_info->register_count; index++) {
		if (register_info->registers[index] != ITEM_Double_2 &&
		    register_info->registers[index] != ITEM_Long_2) {
		    new_entries[index2++] = get_type_code(context, register_info->registers[index]);
		}
	    }
